...
void bigshl(uint64_t *n, size_t shift) {
    if (shift == 0) return;
...
void bigshr(uint64_t *n, size_t shift) {
    if (shift == 0) return;

    size_t word_shift = shift / 64;
    size_t bit_shift = shift % 64;

    for (size_t i = 0; i < S - word_shift; i++) {
        n[i] = (n[i + word_shift] >> bit_shift);
        if (i + word_shift + 1 < S) {
            n[i] |= (n[i + word_shift + 1] << (64 - bit_shift));
        }
    }
    for (size_t i = S - word_shift; i < S; i++) {
        n[i] = 0;
    }
}
...
uint64_t bigdiv(uint64_t *num, uint64_t *den, uint64_t *quo, uint64_t *rem) {
    ...
    size_t shift_amount = num_msb - den_msb;  
